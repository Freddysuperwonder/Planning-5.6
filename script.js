...
function renderAppointments() {
  ...
  sortedAppointments.forEach(appt => {
    ...
    apptElement.innerHTML = `
      <p>${appt.name.toUpperCase()} - ${new Date(appt.date).toLocaleDateString('it-IT', { weekday: 'long', day: 'numeric' })} - ${appt.arrivalTime} - ${appt.appointmentTime} - ${appt.arrivalMode} + ${appt.transport} - <strong>${appt.collaborator.toUpperCase()}</strong></p>
...
function renderPlanning() {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    sortedAppointments.forEach(appt => {
      ...
      apptElement.className = 'appointment';
      apptElement.innerHTML = `
        <p><strong style="color: blue;">${appt.name.toUpperCase()}</strong> - ${appt.arrivalTime} - ${appt.appointmentTime} - ${appt.arrivalMode} + ${appt.transport} - <strong>${appt.collaborator.toUpperCase()}</strong></p>
      `;
